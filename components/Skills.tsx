...
const floatingSkills = [
  { name: 'React', icon: '⚛️', color: 'from-blue-400 to-indigo-400' },
  { name: 'Next.js', icon: '▲', color: 'from-slate-600 to-slate-400' },
  { name: 'Node.js', icon: '🟢', color: 'from-green-500 to-green-400' },
  { name: 'Docker', icon: '🐳', color: 'from-blue-600 to-blue-400' },
  { name: 'Git', icon: '📋', color: 'from-orange-600 to-orange-400' },
            ...
            <div className="relative w-80 h-80">
              {floatingSkills.map((skill, index) => {
                ...
                return (
                  <motion.div
                    key={skill.name}
                    className={`absolute w-12 h-12 rounded-full bg-gradient-to-r ${skill.color} flex items-center justify-center text-white shadow-md text-lg`}
